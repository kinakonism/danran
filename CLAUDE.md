...
### 構成図
```
家族のホーム画面(Worker URL) → Cloudflare Worker → Cloudflare Tunnel → Mac mini: run.py(:8701)
```
...
- **Mac mini の LaunchAgent**（GUIドメイン・KeepAlive/RunAtLoad、`~/Library/LaunchAgents/`）:
  - `com.danran.app` = `uv run python run.py`（env: `PORT=8701`（※TIME_WAIT ゾンビ汚染で 8501→8601（2026-06-07）→8701（2026-07-29）と移転。カーネルの回収停止は再起動でのみ根治・蓄積したら次の空きポートへ） / `STREAMLIT_SERVER_ENABLE_CORS=false`
...
- **再起動時の黒画面対策**: app 再起動で開いている端末の WS が切れ「真っ暗のまま」になるのを防ぐため、
  ...
  健康でもリロードして再接続する。初回ロード詰まり用は7秒ウォッチドッグ（3回即時リロード→以後
  「持久戦モード」: スプラッシュ保持のまま health が通った時だけリロード、間隔 8s→60s 漸増・諦めない）。
  起動ブートスプラッシュ（🏠・スケルトン隠し）も head 注入。上流完全死亡時は素の502でなく
  自動リトライページ（🏠+4秒ごと health 払い→回復で自動リロード）を返す。worker は接続失敗時に
  上流ホストキャッシュを force 更新（トンネル再起動直後の stale 固着を自己修復）。
...
- `backup_daily()` — 全テーブル（users/rooms/room_members/messages/reactions/last_read/push_subscriptions/ai_tasks/ai_reminders/events。sessions は揮発なので対象外）を JSON(gzip) で mini の `~/danran_backups/danran-YYYY-MM-DD.json.gz` に保存・**30世代**保持。同日分はスキップ・失敗時のみまさとに Web Push。**さらに off-site 保険**として `backup_to_r2()` で Worker 経由 R2 非公開バケット `danran-backups` にも PUT（Supabase でも mini 単独でもない3つ目のコピー）。worker ルート `/backup-admin/put`(PUT)・`/backup-admin/list`(GET)・x-danran-auth ゲート・**公開GET配信なし**（password_hash 等を含むため）。
...
### スクローラーの静的探索は実機 iOS で当てにならない

`section.main` / `stMain` 等の候補リストから「overflow がある要素」を探す方式は、実機 iOS で本物のスクロール要素を外すことがある。プル更新🔁はこれで「常に下端」誤判定（どこでも誤発火）と「本物の下端で不発」を起こした（2026-06-10・v159 で修正）。
→ **`scroll` イベント（capture・passive）で実際にスクロールした `e.target` を捕まえて `pDoc._danranScrollEl` にキャッシュ**し、それを最優先で使う。スクロール位置依存の機能（下端判定・ボタン追従）は scan 任せにせず scroll イベントで rAF 間引き追従させる（📷😊ボタンが入力欄から浮く問題の対策と同じ）。
また画像/スタンプのスロットは **min-height による高さ予約が必須**（透明でよい）。予約ゼロだと再描画毎に高さ0→膨張でスクロールが跳ねる。
