...
SYS = (
    ...
    "ための読み取りだけは可）。\n"
    "- 天気・ニュース・最新情報など、リアルタイムの情報が必要な質問は、WebSearch ツールを"
    "使って調べてから回答してください。「分からない」と答える前に必ず検索すること。\n"
    "- 返信は数行で簡潔に、絵文字は控えめに。\n"
...
def build_prompt(msgs, ai_room=True, reminders=""):
    ...
    # リアルタイム情報が必要な質問はプロンプト末尾に強制検索指示を注入
    _REALTIME_KW = ["天気", "気温", "雨", "晴れ", "曇り", "雪", "台風", "降水",
                    "ニュース", "最新", "今日の", "明日の", "今週", "現在の",
                    "何度", "気象", "予報", "株価", "為替", "相場"]
    _latest = (msgs[-1].get("content") or "") if msgs else ""
    _search_note = (
        "\n\n⚠️ 重要: 上の質問はリアルタイム情報が必要です。"
        "返信前に必ず WebSearch ツールで検索してから答えてください。"
        "検索せずに「分からない」と答えることは禁止です。\n"
    ) if any(kw in _latest for kw in _REALTIME_KW) else ""
    if ai_room:
